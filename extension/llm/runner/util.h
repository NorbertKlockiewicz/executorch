 ...
 */
inline ::executorch::runtime::Result<::executorch::extension::TensorPtr>
convert_to_half(const ::executorch::extension::TensorPtr& src_tensor) {
  ET_CHECK_OR_RETURN_ERROR(
      src_tensor->scalar_type() == ::executorch::aten::ScalarType::Float,
      InvalidArgument,
      "Half conversion only supported from Float source data");
  ...
  auto* half_data =
      half_tensor->mutable_data_ptr<::executorch::aten::Half>();
  for (size_t i = 0; i < num_elements; ++i) {
